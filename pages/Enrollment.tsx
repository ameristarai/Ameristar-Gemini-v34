...
import { generateEnrollmentPDF, COURSE_LABELS, PAYMENT_LABELS } from '../utils/enrollmentPDF';
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/.netlify/functions/send-application', {
        ...
        body: JSON.stringify({
          pdfBase64: pdfString,
          studentEmail: formData.email || 'no-email@provided.com'
        }),
      });

      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || 'Failed to securely transmit application.');
      }
