...
// Reads ?page= from the current URL.
// Returns Page.Home when no ?page= param is present (root URL).
// Returns Page.NotFound when a ?page= param exists but is not recognised —
// this is what makes bogus URLs like /?page=xyz show the 404 page instead
// of silently falling back to Home.
const getPageFromUrl = (): Page => {
  const params = new URLSearchParams(window.location.search);
  const pageParam = params.get('page');
  switch (pageParam) {
    case null:         return Page.Home;       // no ?page= → Home
