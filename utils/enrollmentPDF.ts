...
export function generateEnrollmentPDF(data: PDFFormData, totals: PDFTotals): jsPDF {
  ...
  const drawPageHeader = () => {
    ...
    // "School" in gold italic
    doc.setFont('helvetica', 'bolditalic');
    doc.setFontSize(11);
    doc.setTextColor(...GOLD);
    doc.text('School', ML + 39, 13);
  ...
  const drawPageFooter = (pageNum: number, totalPages: number) => {
    ...
    doc.text('Ameristar School  ·  Los Angeles, CA  ·  (626) 308-0150  ·  shirleymiao888@gmail.com', ML, PH - 9);
  ...
  /** Check if we need a new page; if so, draw footer + header and reset y. */
  const checkPage = (neededHeight: number, pageNum: { n: number }) => {
    if (y + neededHeight > PH - 20) {
      drawPageFooter(pageNum.n, 99); // totalPages placeholder — patched after
      doc.addPage();
      pageNum.n++;
      drawPageHeader();
    }
  ...
  field('Payment Method', PAYMENT_LABELS[data.paymentMethod] || data.paymentMethod, pageNum);
  y += 4;

  // ═══════════════════════════════════════════════════════════════════════════
  // SECTION 4: Agreement & Signature (may flow to page 2)
  // ═══════════════════════════════════════════════════════════════════════════
  sectionHeader('04 · Student Agreement & Signature', pageNum);

  // Policy summary box
  checkPage(80, pageNum);
  doc.setFillColor(...LIGHT);
  doc.rect(ML, y, CW, 72, 'F');
  doc.setDrawColor(...GOLD);
  doc.setLineWidth(0.5);
  doc.line(ML, y, ML, y + 72);   // left gold accent bar
  y += 5;
